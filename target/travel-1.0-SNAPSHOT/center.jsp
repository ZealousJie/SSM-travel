    ...
    <div style="margin-left: 150px;">
        <div style="margin-top: 20px;float: left;width: 260px;height:400px;">
            <div style="width: 260px;height: 240px;background-color: #FFFFFF;border: 1px solid #FFFFFF;border-radius: 8px">
                <div style="width: 260px;height: 100px;padding-top: 15px;text-align: center"><img src="<%=basePath%>/${normalUser.upic}" style="width: 60px;height: 55px;margin-top: 20px" id="headPic">
                ...
                <div style="width: 260px;height: 100px;text-align: center;margin-top: 10px">
                    ...
                    <button style="margin: 10px 0;" class="btn4" onclick="zhuXiao()">退出登录</button>
                </div>
            </div>
            <div style="width: 100%;height: 200px;background-color: #FFFFFF;border: 1px solid #FFFFFF;border-radius: 8px;margin-top: 10px;">
                <ul class="isul">
                    <li class="isli" onclick="myOrder()"><a class="isa"> 我的订单</a></li>
        ...
        <div style="float:left;width: 700px;height:700px;margin-top: 20px;margin-left: 20px;background-color: #FFFFFF;border: 1px solid #FFFFFF;border-radius: 8px;">
            <div style="width: 100%;height: 22px;">
                <ul class="isul2">
                    <li class="isli2" onclick="myOrder()"><a class="isa2"> 我的订单</a></li>
            ...
            <div style="width: 700px;height: 600px;" id="detail">
                <form action="${pageContext.request.contextPath}/user/upUser2" enctype="multipart/form-data" method="post">
                    <input type="text" name="id" id="userId" value="${normalUser.id}" hidden/>
                    <input type="text" name="rid" id="rid" value="${normalUser.rid}" hidden/>
                    <div style="width: 700px;height: 90px;padding-top: 10px;text-align:center;margin-top: 10px">
                        <div class="clearfix">
                            <img id="img" src="<%=basePath%>/${normalUser.upic}" height="80px" width="80px" alt="${normalUser.name}" style="border: #00a0e9 1px solid;border-radius: 100%">
                        </div>
                    ...
                    <div style="width: 700px;height: 30px;padding-top: 10px;margin-top: 20px">
                        <span style="display: block;margin-left: 80px"><label style="font-size: 14px;margin-right: 20px;font-weight: normal">姓名:</label><input type="text" value="${normalUser.name}" class="in" name="name" ></span>
                    </div>
                    <div style="width: 700px;height: 30px;padding-top: 10px;margin-top: 20px">
                    ...
                    <div style="width: 700px;height: 30px;padding-top: 10px;margin-top: 20px">
                        <span style="display: block;margin-left: 95px"><label style="font-size: 14px;margin-right: 20px;font-weight: normal">手机号:</label><input type="text" value="${normalUser.phone}" class="in" name="phone"></span>
                    </div>
                    <hr style="height: 1px;width: 660px;border: 0;background-color: #e5e9ef;margin-top: 40px;margin-left: 20px">
...
<script>
    $(
        function (){
            if(${normalUser != null}){
                ...
                name.innerHTML = '${normalUser.name}';
                <%--name.innerHTML = '${normalUser.name}';--%>
                headPro.src = "<%=basePath%>/${normalUser.upic}";
                <%--headPro.src = "<%=basePath%>/${normalUser.upic}";--%>
                img.src = "<%=basePath%>/${normalUser.upic}";
                <%--img.src = "<%=basePath%>/${normalUser.upic}";--%>
            }
                ...
                if(data !== null){
                    for(let i=0;i<data.length;i++){
                        ...
                        div.innerHTML = "<div style=\"width: 700px;height: 50px;padding-top: 10px;margin-top: 10px;\">"+
                            "                  <label style=\"font-size: 14px;font-weight: unset;margin-left: 20px;color: #333333\">"+data[i].name+"</label>"+
                            "                   <label style=\"font-size: 12px;font-weight: lighter;margin-left: 15px;color:#9F9F9F;\">"+"价格："+data[i].price+"</label>"+
                            "                   <div style=\"margin-top: 10px\">"+
                            "                   <label style=\"margin-left: 40px;color: #9a9a9a;font-size: 13px\">"+"出行时间："+data[i].ttime+"</label>" +
                            "                   <label style=\"margin-left: 40px;color: #9a9a9a;font-size: 13px\">"+"下单时间："+data[i].odate+"</label>" +
                            "                   </div></div>"+
                            "<hr style=\"height: 1px;width: 700px;border: 0;background-color: #e5e9ef\">";
                    ...
                    advisor.innerHTML = "<div style=\"width: 700px;height: 100px;padding-top: 10px;margin-top: 10px;text-align: center\">"+
                        "               <label style=\"margin-left: 40px;color: #9a9a9a;font-size: 13px\">暂无订单</label></div>";
                }
    ...
    function zhuXiao(){
        $.ajax({
            dataType:"json",
            url:"${pageContext.request.contextPath}/zhuXiao1",
            type:"get",
            success:function (){
                location.href="<%=basePath%>/admin/all-admin-login.html"
                alert("退出登录成功")
            }
        })
    }
